...
void signal_T2(void *pvParameters);
void poll_T3(void *pvParameters);
void poll_T4(void *pvParameters);
void call_T5(void *pvParameters);
void sum_T6(void *pvParameters);
void b_T7(void *pvParameters);
...
void IRAM_ATTR b_T7_ISR() {
  ...
  if (currentTime - lastDebounceTime > debounceDelay)
  {
    ...
    //once ISR is done, run higher priority task immediately 
    if (higherPriorityTaskWoken)
    {
      //runs higher priority task if there is one 
      portYIELD_FROM_ISR();
    }
  }
}
...
void setup() {
  ...
  xTaskCreate(signal_T1, "T1_Signal", 2048, NULL, 3, &task1Handle);
  
  // Task 2 - 333Hz (3ms period)
  xTaskCreate(signal_T2, "T2_Signal", 2048, NULL, 4, &task2Handle);
  
  // Task 3 - 100Hz (10ms period)
  xTaskCreate(poll_T3, "T3_Poll", 2048, NULL, 2, &task3Handle);
  
  // Task 4 - 100Hz (10ms period)
  xTaskCreate(poll_T4, "T4_Poll", 2048, NULL, 2, &task4Handle);
  
  // Task 5 - 200Hz (5ms period)
  xTaskCreate(call_T5, "T5_DoWork", 2048, NULL, 3, &task5Handle);
  
  // Task 6 - checks frequency sum (no hard deadline, lower priority)
  xTaskCreate(sum_T6, "T6_Sum", 2048, NULL, 1, &task6Handle);
  
  // Task 7 - handles button presses (event-driven)
  xTaskCreate(b_T7, "T7_Button", 1024, NULL, 1, &task7Handle);
}

void signal_T1(void *pvParameters) {
  // Get current tick count for vTaskDelayUntil
  TickType_t xLastWakeTime = xTaskGetTickCount();
  const TickType_t xFrequency = pdMS_TO_TICKS(4); // 4ms period 
...
void signal_T2(void *pvParameters) {
  // Get current tick count for vTaskDelayUntil
  TickType_t xLastWakeTime = xTaskGetTickCount();
  const TickType_t xFrequency = pdMS_TO_TICKS(3); // 3ms period 
...
void call_T5(void *pvParameters) {
  // Get current tick count for vTaskDelayUntil
  TickType_t xLastWakeTime = xTaskGetTickCount();
  const TickType_t xFrequency = pdMS_TO_TICKS(5); // 5ms period 
